...
public class UnixDomainSocketClient implements AutoCloseable {
    ...
    /**
     * UnixDomainSocketClientのコンストラクタ。
     * シングルスレッドの ExecutorService を初期化します。
     */
    ...
    /**
     * サーバーに接続します。
     * @throws IllegalStateException ソケットが作成されていない場合
     ...
     * @throws NotYetConnectedException ソケットが接続されていない場合
    ...
    /**
     * サーバーにリクエストを送信し、処理結果を待ち受けます。
     * このメソッドは、リクエストの送信から結果の受信までを一連の操作として実行します。
     * タイムアウトを設定し、指定された時間内に結果が返ってこない場合は例外をスローします。
     *
     * @param request サーバーに送信するリクエストデータを含むByteBuffer
     * @param timeout 結果を待つ最大時間（ミリ秒）
     * @return サーバーでの処理が成功した場合はtrue、失敗した場合はfalse
     * @throws IOException 通信中にエラーが発生した場合
     * @throws NotYetConnectedException ソケットが接続されていない場合
     * @throws TimeoutException 指定された時間内に結果が返ってこなかった場合
     */
